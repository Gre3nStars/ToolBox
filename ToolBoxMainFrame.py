...
LastEditors: Author
LastEditTime: 2025-08-15 14:24:53
Description: 
...
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QVBoxLayout, QPushButton, QTreeWidget, QTreeWidgetItem, QSplitter,
    QToolButton, QTabWidget, QFrame, QGridLayout, QTreeWidgetItemIterator, QFileDialog, QMessageBox, QTextEdit,
    QFontDialog, QSystemTrayIcon, QStyle, QMenu
)
from PySide6.QtCore import QEasingCurve, Signal
from PySide6.QtGui import QIcon, QAction

from widgets.system_status_bar import SystemStatusBar
# 导入所有子模块



# 全局
app_name  = "ToolBox 公开版"
# app_ver = "V1.0.10" #2025.07.31
app_ver = "V1.0.11" #2025.08.15
app_user = "Gre3nStar"
...
class CollapsibleSidebar(QWidget):
    ...
    def add_menu_items(self):
        ...
        gmssl_item = QTreeWidgetItem(crypto_item)
        gmssl_item.setText(0, "国密算法工具")
        gmssl_item.setIcon(0, QIcon(normal_icon_path))
        ...
        hmac_item = QTreeWidgetItem(crypto_item)
        hmac_item.setText(0, "HMAC工具")
        hmac_item.setIcon(0, QIcon(normal_icon_path))

        bcrypt_item = QTreeWidgetItem(crypto_item)
        bcrypt_item.setText(0, "Bcrypt加密工具")
        bcrypt_item.setIcon(0, QIcon(normal_icon_path))
        ...
        jsonconvert_item = QTreeWidgetItem(dev_item)
        jsonconvert_item.setText(0, "JsonConvert工具")
        jsonconvert_item.setIcon(0, QIcon(normal_icon_path))
        ...
        virus_dect_item = QTreeWidgetItem(penetration_item)
        virus_dect_item.setText(0, "杀软查询工具")
        virus_dect_item.setIcon(0, QIcon(normal_icon_path))

        icp_query_item = QTreeWidgetItem(penetration_item)
        icp_query_item.setText(0, "ICP备案查询工具")
        icp_query_item.setIcon(0, QIcon(normal_icon_path))

        subdomain_query_item = QTreeWidgetItem(penetration_item)
        subdomain_query_item.setText(0, "子域名查询工具")
        subdomain_query_item.setIcon(0, QIcon(normal_icon_path))
        ...
        ip_location_item = QTreeWidgetItem(others_item)
        ip_location_item.setText(0, "IP查询工具")
        ip_location_item.setIcon(0, QIcon(normal_icon_path))

        ip_calculator_item = QTreeWidgetItem(others_item)
        ip_calculator_item.setText(0, "IP计算器工具")
        ip_calculator_item.setIcon(0, QIcon(normal_icon_path))


        random_chr_item = QTreeWidgetItem(others_item)
        random_chr_item.setText(0, "随机字符生成工具")
        random_chr_item.setIcon(0, QIcon(normal_icon_path))
    ...
    def on_item_clicked(self, item, column):
        ...
        if not item_text:
            item_text = item.data(0, Qt.ItemDataRole.UserRole)
    ...
    def toggle_collapse(self):
        if self.is_collapsed:
            ...
            while iterator.value():
                item = iterator.value()
                original_text = item.data(0, Qt.ItemDataRole.UserRole)
                if original_text:
...
class MainWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        font_action.setIcon(QIcon.fromTheme('preferences-desktop-font'))
        ...
        # 新建临时记事本
        new_note_action = QAction("新建记事本", self)
        ...
        open_note_action.triggered.connect(self.open_file)
        notepad_menu.addAction(open_note_action)

        # 保存记事本
        save_note_action = QAction("保存记事本", self)
        save_note_action.setIcon(QIcon.fromTheme('document-save'))
        # save_note_action.setShortcut("Ctrl+S")
        save_note_action.triggered.connect(self.save_file)
        notepad_menu.addAction(save_note_action)
    ...
    def show_about_dialog(self):
        ...
        QMessageBox.about(self,
            "关于本程序",
            f"ToolBox 公开版，欢迎使用，提bug，提使用建议！\n版本: {app_ver}\n© 2025 {app_user}"
        )

    def new_notepad(self, title="", content=""):
        text_page = QWidget()
        ...
        # 确定标签页标题
        if not title:
            title = f"临时记事本 {self.tab_widget.count() + 1}"

        # 添加标签页并切换到新标签页
        self.tab_widget.addTab(text_page, title)
    ...
    def open_file(self):
        ...
        if file_path:
            try:
                ...
                # 创建新标签页并显示文件内容
                self.new_notepad(title=file_name, content=content)
            except Exception as e:
                print(f"打开文件失败: {e}")
    ...
    def save_file(self):
        ...
        # 获取当前标签页的文本编辑控件
        text_page = self.tab_widget.widget(current_index)
        text_edit = text_page.findChild(QTextEdit)
        if not text_edit:
            return
        ...
        # 打开文件保存对话框
        file_path, _ = QFileDialog.getSaveFileName(
            self, "保存文件", current_title, "文本文件 (*.txt);;所有文件 (*)"
        )
    ...
    def open_new_tab(self, tab_title):
        ...
        if tab_title == "加密解密工具":
            ...
            from widgets.encrypt_widget import EncryptToolWindow
            new_page = EncryptToolWindow(self)  # 实例化外部窗口
        elif tab_title == "编码解码工具":
            from widgets.codec_widget import CodecWidget
            new_page = CodecWidget()
        elif tab_title == "RSA加解密工具":
            from widgets.rsa_widget import RSAtool
            new_page = RSAtool()
        elif tab_title == "MD5工具":
            from widgets.md5_widget import EncryptionTool
            new_page = EncryptionTool()
        elif tab_title == "HMAC工具":
            from widgets.hmac_widget import HmacTool
            new_page = HmacTool()
        elif tab_title == "Bcrypt加密工具":
            from widgets.bcrypt_widget import BcryptEncryptor
            new_page = BcryptEncryptor()
        elif tab_title == "国密算法工具":
            from widgets.guomi_widget import GMCryptoTool
            new_page = GMCryptoTool()
        elif tab_title == "Json处理工具":
            from widgets.json_widget import JsonProcessor
            new_page = JsonProcessor()
        elif tab_title == "JsonConvert工具":
            from widgets.jsonconvert_widget import JsonConverter
            new_page = JsonConverter()
        elif tab_title == "JWT处理工具":
            from widgets.jwt_widget import JWTUtility
            new_page = JWTUtility()
        elif tab_title == "Hosts修改工具":
            from widgets.hosts_widget import HostsTool
            new_page = HostsTool()
        elif tab_title == "二维码工具":
            from widgets.qrcode_widget import QRCodeTool
            new_page = QRCodeTool()
        elif tab_title == "常用命令工具":
            from widgets.command_widget import CommandTool
            new_page = CommandTool()
        elif tab_title == "ICP备案查询工具":
            from widgets.icp_query_widget import ICPInquiryTool
            new_page = ICPInquiryTool()
        elif tab_title == "子域名查询工具":
            from widgets.subdomain_query_widget import MultiDNSQueryTool
            new_page = MultiDNSQueryTool()
        elif tab_title == "正则表达式工具":
            from widgets.regex_widget import RegexTool
            new_page = RegexTool()
        elif tab_title == "Unix时间戳工具":
            from widgets.timestamp_widget import TimestampTool
            new_page = TimestampTool()
        elif tab_title == "IP查询工具":
            from widgets.ip_location_widget import IPLocationTool
            new_page = IPLocationTool()
        elif tab_title == "IP计算器工具":
            from widgets.ip_calculator_widget import IPAddressCalculator
            new_page = IPAddressCalculator()

        elif tab_title == "杀软查询工具":
            from widgets.virus_dect_widget import AntivirusDetectorTool
            new_page = AntivirusDetectorTool()

        elif tab_title == "随机字符生成工具":
            from widgets.random_chr_widget import RandomCharGenerator
            new_page = RandomCharGenerator()
    ...
    def close_tab(self, index):
        ...
        self.tab_widget.removeTab(index)
        tab_content.destroy()
        tab_content.deleteLater()  # 确保部件被正确删除
    ...
    def confirm_quit(self):
        ...
        for i in range(self.tab_widget.count()):
            ...
            if hasattr(tab_content, 'thread') and tab_content.thread:
                tab_content.thread().deleteLater()
                tab_content.thread().exit(0)

        self.tray_icon.hide()
        QApplication.instance().quit()
    
    # 重写closeEvent，增加关闭确认 2025/8/7 更新窗口关闭确认
    # 再次重写，增加最小化系统托盘 2025/8/8
    def closeEvent(self, event):
        """主窗口关闭时显示确认对话框，支持退出或最小化到托盘，并在退出时停止所有线程"""
        # 检查系统是否支持托盘功能
        if QSystemTrayIcon.isSystemTrayAvailable() and self.tray_icon:
            # 创建包含最小化和退出选项的对话框
            msg_box = QMessageBox(self)
            msg_box.setWindowTitle("关闭程序")
            ...
            if msg_box.clickedButton() == quit_btn:
            ...
            elif msg_box.clickedButton() == minimize_btn:
                # 最小化到托盘
                self.hide()
                # 显示托盘提示
                if hasattr(self, 'tray_icon') and self.tray_icon:
                    self.tray_icon.showMessage(
                        "程序最小化",
                        "程序已最小化到系统托盘",
                        QSystemTrayIcon.Information,  # 正确的消息图标类型
                        1000  # 显示1秒
                    )
                event.ignore()  # 忽略关闭事件
...
QCheckBox::indicator:checked {
    background-color: #28a745; /* 选中背景绿色 */
    border-color: #28a745;     /* 选中边框绿色 */
}
    ...
    window = MainWindow()
    window.show()
    window.setWindowTitle("ToolBox 公开版")
    sys.exit(app.exec())
