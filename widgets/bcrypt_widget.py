...
from PySide6.QtCore import Qt, QThread, Signal, QObject, QRunnable, QThreadPool, Slot, QMutex, QMutexLocker
...
class BruteForceWorker(QRunnable):
    def __init__(self, hash_bytes, candidates, stop_flag, result_callback, progress_callback):
        super().__init__()
        self.hash_bytes = hash_bytes
        self.candidates = candidates  # list of candidate strings
        self.stop_flag = stop_flag    # QMutex-protected bool
    ...
    def run(self):
        for candidate in self.candidates:
            ...
            if not candidate:
                continue
            try:
                if bcrypt.checkpw(candidate.encode("utf-8"), self.hash_bytes):
                    self.result_callback(candidate)
...
class BruteForceThread(QThread):
    ...
    def __init__(self, hash_text, dict_path, num_threads=20):
        super().__init__()
        ...
        self._found = False
        self._stop_flag = {'stopped': False}
        self._mutex = QMutex()
        self._total = 0
        self._current = 0
        self._threadpool = QThreadPool()
        self._threadpool.setMaxThreadCount(self.num_threads)
    ...
    def run(self):
        try:
            ...
            if self._total == 0:
                ...
                return
            # 分片
            chunk_size = (self._total + self.num_threads - 1) // self.num_threads
            self._current = 0
            self._found = False
            self._stop_flag['stopped'] = False
            self._result = None

            def result_callback(candidate):
            ...
            for i in range(self.num_threads):
                start = i * chunk_size
                end = min((i + 1) * chunk_size, self._total)
                if start >= end:
                    continue
            ...
            if not self._found and not self._stop_flag['stopped']:
                self.result.emit("")
            self.finished.emit()
