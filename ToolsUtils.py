...
from PySide6.QtWidgets import QTextEdit, QLineEdit, QStatusBar, QLabel
import re
import urllib.parse
...
class ToolUtils:
    ...
    def update_textChanged_statusbar(input_widget:(QTextEdit,QLineEdit),output_widget:(QTextEdit,QLineEdit),status_bar:(QLabel)):
        ...
        intput_line_num = input_widget.document().blockCount()
        output_len = len(output_widget.toPlainText())
        output_line_num = output_widget.document().blockCount()
        # status_bar.setStyleSheet("QStatusBar::item { alignment: AlignRight; }")
        status_bar.setText(f"当前输入字符长度为：{input_len}，行数：{intput_line_num}，输出字符长度为：{output_len}，行数：{output_line_num}")
